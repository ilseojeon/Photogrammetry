...
std_IP = sscanf( '0.0005', '%f' ); %% Standard deviation of image point measurement error [mm]
...
%     for ni = 1:no_EO_c,
%         imi = find ( id_IM == EO_c(ni,1) );
%         for nc = 1:6,
%             Ke((ni-1)*6+nc, (imi-1)*6+nc) = 1;
%             ye((ni-1)*6+nc, 1) = EO_c(ni,nc+1) - Kt_e((imi-1)*6+nc,1);
%         end
%     end
...
%     % Set the normal matrix(GP)
    Nee = Ae' * Pi * Ae;
    Neg = Ae' * Pi * Ag;
    Ngg = Ag' * Pi * Ag + Kg' * Pg * Kg;
    Ce = Ae' * Pi * yi;
    Cg = Ag' * Pi * yi + Kg' * Pg * yg;
    
    % Set the normal matrix(EO + GP)
%     Nee = Ae' * Pi * Ae + Ke' * Pe * Ke;
%     Neg = Ae' * Pi * Ag;
%     Ngg = Ag' * Pi * Ag + Kg' * Pg * Kg;
%     Ce = Ae' * Pi * yi + Ke' * Pe * ye;
...
A = [Ae Ag; zeros(size(Kg,1),size(Ae,2)) Kg];
y = [yi; yg];

P = zeros(length(Pi)+length(Pg));
P(1:length(Pi),1:length(Pi)) = Pi;
P(length(Pi)+1:length(Pi)+length(Pg),length(Pi)+1:length(Pi)+length(Pg)) = Pg;
